...
mongoose
  .connect(
    "mongodb+srv://mohdabuzaid0132:dbUserPassword@threadscluster.qa7wg.mongodb.net/",
    {
...
const sendVerificationEmail = async (email, verificationToken) => {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "mohdabuzaid0132@gmail.com",
      pass: "oswp mxxl atye jwyl",
    },
  });
  ...
  const mailOptions = {
    ...
    text: `Please click the following link to verify your email: http://192.168.43.210:3000/verify/${verificationToken}`,
  };
...
app.put("/posts/:postId/:userId/like", async (req, res) => {
  ...
  try {
    // Find and populate the post first
    ...
    const updatedPost = await Post.findByIdAndUpdate(
      ...
      { new: true }
    ).populate("user", "name"); // Populate user field after updating
